...
/** Bilingual string. Index with the current language code. */
...
function SpeakerBox({ heading, children }: { heading: L; children: ReactNode }) {
  ...
  return (
    <div className="mb-8 p-6 bg-card border border-border rounded-lg">
      <h3 className="text-xl font-semibold text-foreground mb-4">{heading[language]}</h3>
      <div className="space-y-3">{children}</div>
    </div>
  );
}

/** Bulleted list used by "Talk Themes" and "Event Highlights". */
function BulletList({ items }: { items: L[] }) {
  const { language } = useLanguage();
  return (
...
/** Date & time detail block (with clock icon). `note` renders an extra grey line. */
function DateTime({ date, time, note }: { date: L; time: L; note?: L }) {
  const { language } = useLanguage();
  return (
    <div className="flex gap-4">
      <Clock className="w-6 h-6 text-primary flex-shrink-0 mt-1" />
      <div>
        <h4 className="font-semibold text-foreground mb-2">{language === "en" ? "Date & Time" : "日時"}</h4>
        <p className="text-muted-foreground text-sm">
          {date[language]}
          <br />
          <span className="text-foreground font-semibold">{time[language]}</span>
          {note && (
            <>
              <br />
              <span className="text-sm text-muted-foreground mt-1 block">{note[language]}</span>
            </>
          )}
        </p>
      </div>
...
/** Venue detail block — identical for every meeting (same place + map link). */
function Venue() {
  const { language } = useLanguage();
  return (
    <div className="flex gap-4">
      ...
      <div>
        ...
        <p className="text-muted-foreground text-sm mb-2">Board game cafe bar Alternative</p>
        <a
...
/** Standard fee table (incl. networking reception) — shared by the 2nd & 3rd meetings. */
function FeeTable() {
  const { language } = useLanguage();
  const rows: { label: L; value: ReactNode }[] = [
    { label: { en: "Regular / Associate members", ja: "正会員・準会員" }, value: <span className="text-foreground">¥5,000</span> },
    { label: { en: "Student members", ja: "学生会員" }, value: <span className="text-primary">{language === "en" ? "Free" : "無料"}</span> },
    { label: { en: "Non-members (general)", ja: "非会員（一般）" }, value: <span className="text-foreground">¥8,000</span> },
    { label: { en: "Non-members (students)", ja: "非会員（学生）" }, value: <span className="text-foreground">¥5,000</span> },
  ];
  return (
    <div className="mb-8">
      <div className="flex gap-4">
        ...
        <div className="flex-1">
          ...
          <div className="mb-3 text-sm text-muted-foreground">
            <table className="w-full border-collapse">
              ...
              <tbody>
                {rows.map((row, i) => (
                  <tr key={i} className={i < rows.length - 1 ? "border-b border-amber-100" : undefined}>
                    <td className="py-1.5 pr-4">{row.label[language]}</td>
                    <td className="text-right py-1.5 font-semibold">{row.value}</td>
                  </tr>
                ))}
              </tbody>
            </table>
...
/** Highlights for the 1st (April, ended) meeting. */
const APRIL_HIGHLIGHTS: L[] = [
  { en: "Unique career path: IPO experience and leading a listed company from Singapore", ja: "上場経験＆シンガポールからの上場企業経営！異色のキャリアから見えた世界と日本" },
  { en: 'Global perspective on "Japan\'s regions and Kagawa\'s current state and potential"', ja: "グローバル視点から紐解く「日本の地方と香川の現状と可能性」" },
  { en: 'Future challenges: "My next steps and direction"', ja: "これからの挑戦「今後の自分の身の振り方」" },
];
    ...
    <div className="min-h-screen bg-white">
      ...
      <section className="py-20 bg-white">
        <div className="container max-w-4xl">
          <h1 className="text-4xl font-bold text-primary mb-4">{language === "en" ? "News" : "ニュース"}</h1>
          ...
          <Card className="p-6 mb-8 bg-gradient-to-r from-purple-50 to-pink-50 border border-purple-200">
            <a href="https://www.instagram.com/kaibkagawa/" target="_blank" rel="noopener noreferrer" className="flex items-center gap-4 group">
              ...
              <div className="flex-1">
                ...
                <p className="text-sm text-muted-foreground">@kaibkagawa</p>
              </div>
